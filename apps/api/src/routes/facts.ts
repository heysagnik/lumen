...
factsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [fact] = await db.select(factColumns).from(facts).where(eq(facts.id, id)).limit(1);
    if (!fact) {
    ...
    const [chunk] = await db
      .select({ pageNumber: chunks.pageNumber, text: chunks.text })
      .from(chunks)
      .where(eq(chunks.id, fact.chunkId))
      .limit(1);
    ...
    const relatedFacts = relatedFactIds.length
      ? await db.select(factColumns).from(facts).where(inArray(facts.id, relatedFactIds))
      : [];
